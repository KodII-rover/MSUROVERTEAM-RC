...
# **Интеграция с камерой глубины.**

«Библиотека автономной навигации по распознанным указателям движения» интегрирована с камерой глубины, установленной на ровере и подключенной к его главному компьютеру. При движении ровера, в режиме реального времени данные с камеры глубины поступают в «Библиотеку автономной навигации по распознанным указателям движения».

- Входные данные, получаемые с камеры глубины:
	- топик /camera/camera/depth/image_rect_raw — выравненное и калиброванное изображение глубины (карта расстояний),
 	- топик /camera/camera/aligned_depth_to_color/image_raw — глубина, наложенная на RGB-кадр,
	- топик /camera/camera/depth/color/points — 3D-облако точек, которое используется блоками навигации (Nav2) для обхода препятствий.

В нашем ровере мы использовали камеру глубины Intel RealSense d435i. Чтобы предавать данные с этой камеры глубины только в объеме указанном выше, мы  запускаем ее официальный драйвер ('realsense2_camera') через дополнительно разработанный нами пакет <a href="https://github.com/andreyalex1/eureka_camera_2">'eureka_camera_2'</a>. Данный пакет не является частью проекта «Разработки открытых библиотек для автономной навигации шестиколесного прототипа марсохода (ровера) по умеренно пересечённой незнакомой местности с визуальным распознаванием цели навигации», а процесс его установки приведен в данной инструкции для примера. Поэтому все команды установки камеры глубины RealSense в данной инструкции имеют пометку в комментариях: '# ДЛЯ ПРИМЕРА!!!'.
...
Здесь устанавливаем официальный драйвер камеры глубины:

```
# ДЛЯ ПРИМЕРА!!! Устанавливаем официальный драйвер камеры RealSense для ROS 2
...
Клонируем репозиторий библиотеки и пакет запуска камеры в папку src:

```
# Клонирование основной библиотеки SLAM
git clone https://github.com/KodII-rover/MSUROVERTEAM-SLAM.git

# ДЛЯ ПРИМЕРА!!! Клонирование пакета управления камерой глубины RealSense eureka_camera_2
git clone https://github.com/andreyalex1/eureka_camera_2.git
```
...
# Соберираем все остальные пакеты рабочего пространства
colcon build
```
...
# ДЛЯ ПРИМЕРА!!! Повторно собираем пакет работы с камерой глубины RealSense
colcon build --packages-select eureka_camera_2

# Повторно собираем остальные пакеты
...
<strong>ВНИМАНИЕ!!! Если к основному компьютеру ровера НЕ ПОДКЛЮЧЕНО периферийное оборудование описанное в разделе "МИНИМАЛЬНЫЙ ТЕХНИЧЕСКИЕ ТРЕБОВАНИЯ", или подключено оборудование ОТЛИЧНОЕ от того, которое использовалось при работе над проектом, то в процессе запуска "автономной навигации по распознанным указателям движения" возникнут ошибки.</strong>
