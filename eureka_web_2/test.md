...
* [Интеграция с камерой глубины](#интеграция-с-камерой-глубины)
...
* [Альтернативный пример программного кода](#альтернативный-пример-программного-кода-для-автономного-движения-ровера-до-целевых-точек-с-указанием-целевой-позиции)
...
- ackermann-msgs,
- ROS2 (интеграция с роботом/ровером).
...
Устанавливаем официальный драйвер камеры глубины.
<strong>ВНИМАНИЕ! При установке официального драйвера камеры глубины необходимо строго следовать инструкции производителя устройства и разработчика SDK.</strong>

```
# ДЛЯ ПРИМЕРА!!! Устанавливаем официальный драйвер камеры RealSense для ROS 2
sudo apt install ros-humble-realsense2-camera

# Процесс установки официального драйвера камеры глубины Intel RealSense зависит от модели камеры и главного компьютера ровера, 
# поэтому выполнения одной лишь упомянутой выше команды может быть недостаточно - она может завершиться ошибками. 
# При установке драйвера необходимо строго следовать официальной инструкции производителя и разработчика SDK. 
...
<strong>ВНИМАНИЕ! Если к главному компьютеру ровера не подключено периферийное оборудование, описанное в разделе «Минимальные технические требования», или подключено оборудование, отличное от использованного при разработке проекта, то в процессе запуска библиотеки автономной навигации по распознанным указателям движения возникнут ошибки. 
Также необходимо убедиться, что официальные ROS2-драйверы аппаратного обеспечения (включая камеру глубины) установлены корректно, запускаются и публикуют данные в соответствующие ROS2-топики независимо от данной библиотеки. Если эти требования не выполнены, то при запуске библиотеки автономной навигации по распознанным указателям движения также возникнут ошибки.</strong>
